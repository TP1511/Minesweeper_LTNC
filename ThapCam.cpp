...
bool init() {
	...
	if (SDL_Init(SDL_INIT_VIDEO) < 0) {
		...
		//Create window
		gWindow = SDL_CreateWindow("Tuan Phuong's Minesweeper", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, SCREEN_WIDTH, SCREEN_HEIGHT, SDL_WINDOW_SHOWN);
		if (gWindow == NULL) {
...
bool loadmedia(){
	...
	if (!gTilesSpriteSheetTexture.loadFromFile("Tiles.png")){
		...
		//Set sprites
		for (int i=0;i<12;i++) {	//toa do va kich thuoc cua o tile trong game
	...
	if (!gDigitSpriteSheetTexture.loadFromFile("numbers.png")){
		...
		//Set sprites
		for (int i = 0;i < 10;i++) //toa do va kich thuoc cua o hien so min con lai
		{
	...
	SDL_Color textcolor = { 207,3,252 };//tim
			...
			if (event.type == SDL_MOUSEBUTTONDOWN || event.type == SDL_MOUSEMOTION){
				...
				if (event.type == SDL_MOUSEBUTTONDOWN && event.button.button == SDL_BUTTON_LEFT){
                    if (easyInside){
                        ...
                        setGameMode(9, 9, 10, 2, 2, 28*9+42-56, BOARD_SIZE_X, BOARD_SIZE_Y, sumOfMines, minesLeft, unopenedTilesLeft, digit_x, digit_y, face_x);
                        CreateBoard();
                        ...
                        setGameMode(16, 16, 40, 2, 2, 28*16+42-56, BOARD_SIZE_X, BOARD_SIZE_Y, sumOfMines, minesLeft, unopenedTilesLeft, digit_x, digit_y, face_x);
                        CreateBoard();
                    } else if(hardInside){
                        isRunning = true;
                        isChoosingMode = false;
                        ...
                        setGameMode(30, 16, 99, 2, 2, 28*30+42-56, BOARD_SIZE_X, BOARD_SIZE_Y, sumOfMines, minesLeft, unopenedTilesLeft, digit_x, digit_y, face_x);
                        CreateBoard();
                    }
				}
	...
	if(boardForShow[i][j] > 0 && boardForShow[i][j] < 9) { 
        ...
        for(int i1 = -1; i1 < 2; i1++){
            for(int j1 = -1; j1 < 2; j1++){
                ...
                if(boardForShow[i+i1][j+j1] == 11) countFlags++;
            }
        }
        if(countFlags == board[i][j]) { 
            for(int i1 = -1; i1 < 2; i1++){
                for(int j1 = -1; j1 < 2; j1++){
                    if(i+i1 < 0 || i+i1 > BOARD_SIZE_X-1) continue;
                    if(j+j1 < 0 || j+j1 > BOARD_SIZE_Y-1) continue;
...
void GameManager(){
	if(playAgain) {
        ...
        CreateBoard();
        Mix_HaltMusic();
        minesLeft = sumOfMines;
        unopenedTilesLeft = BOARD_SIZE_X * BOARD_SIZE_Y;
        winGame = false;
        loseGame = false;
        playAgain = false;
	}
...
void showingMineLeft(){
	if(minesLeft < 10){
		...
		for(int i = 0;i <= 9;i++){
			if(i == minesLeft) gDigitSpriteSheetTexture.render(digit_x + 28, digit_y, &gSpriteClipsForDigit[i]);
		}
	} else {
	    int n = minesLeft;
		int i = 0;
...
void setGameMode(int boardsizex, int boardsizey, int mine, int dx, int dy, int fx, int& BOARD_SIZE_X, int& BOARD_SIZE_Y, int& sumOfMines, int& minesLeft, int& unopenedTilesLeft, int& digit_x, int& digit_y, int& face_x){
	BOARD_SIZE_X = boardsizex;
	BOARD_SIZE_Y = boardsizey;
	sumOfMines = mine;
	minesLeft = mine;
	unopenedTilesLeft = boardsizex*boardsizey;
	digit_x = dx;
	digit_y = dy;
	face_x = fx;
